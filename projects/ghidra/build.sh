...
./gradlew --no-daemon -x test -x integrationTest buildGhidra

mkdir -p "$OUT/jars" "$OUT/lib" "$WORK/fuzzer-classes"
copy_jars() {
  local source_dir="$1"
  local dest_dir="$2"
...
copy_libs() {
  ...
  local dest_dir="$2"
  [ -d "$source_dir" ] || return 0
  find "$source_dir" -type f \( -name '*.so' -o -name '*.so.*' \) -print0 | sort -z | while IFS= read -r -d '' lib; do
    cp "$lib" "$dest_dir/$(basename "$lib")" || true
  done
}
copy_jars "$PWD" "$OUT/jars"
copy_jars "$GRADLE_USER_HOME/caches/modules-2/files-2.1" "$OUT/jars"
copy_libs "$PWD" "$OUT/lib"
